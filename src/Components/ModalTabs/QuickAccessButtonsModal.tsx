...
type QuickAccessButtonsModalProps = {
  isOpen: boolean;
  onClose: () => void;
  title: string;
  form: quickAccessButtonsForm;
  errors: any;
  setErrors: any;
  setForm: React.Dispatch<React.SetStateAction<quickAccessButtonsForm>>;
  onSubmit: () => void;
  mode: "create" | "edit";
          ...
          <div className={getClass("modalBodyContainer")}>
            <fieldset className={getClass("category")}>
              ...
              <div className={getClass("field")}>
               ...
               <div className={getClass("field")}>
                ...
                >
                  ...
                  <option value={"above_group"}>Parte superior</option>
                  <option value={"under_group"}>Parte inferior</option>
                </select>
              ...
              <div className={getClass("field")}>
                ...
                <input
                  className={`${getClass("input")} ${errors.link ? getClass("inputError") : ""}`}
                  value={form.link}
                  placeholder="Digite o link de redirecionamento"
                  maxLength={255}
                  onChange={(e) => {handleChange(e.target.value, "link")
                     if (errors.link)
                      setErrors((prev: any) => ({ ...prev, link: false }));
                  }}
                />
                 {errors.link && (
                  <p className={getClass("errorText")}>Link obrigatório</p>
                )}
              </div>
            </fieldset>
...
export default QuickAccessButtonsModal;
